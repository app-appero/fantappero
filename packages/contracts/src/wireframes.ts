/** Wireframe catalog copy and metadata (EPUI-04) — shared by web and mobile shells. */
...
export type WireframeUiState = (typeof WIREFRAME_STATES)[number];
...
export const WIREFRAME_CATALOG: readonly WireframeScreenMeta[] = [
  ...
  {
    id: "matchday",
    title: "Turni H2H e europei",
    route: "/turni",
    macroflows: ["M2", "M3"],
    primaryCta: "Consulta scontri",
    priorityInfo: ["Calendario fantallenatori", "Risultati H2H", "Turni europei"],
    criticalSteps: [
      "Calendario fantallenatori",
      "Dettaglio scontro",
      "Turni europei per competizione",
    ],
    roles: ["Partecipante"],
    empty: {
      title: "Calendario non pronto",
      description: "La stagione non è ancora iniziata o il calendario H2H non è confermato.",
      ctaLabel: "Vai ad Amministrazione",
    },
    error: {
      title: "Turni non disponibili",
      message: "Impossibile caricare calendario e turni europei.",
      retryLabel: "Ricarica",
    },
    loading: {
...
export function parseWireframeState(value: string | null | undefined): WireframeUiState {
  if (value && (WIREFRAME_STATES as readonly string[]).includes(value)) {
    return value as WireframeUiState;
  }
  return "success";
}
