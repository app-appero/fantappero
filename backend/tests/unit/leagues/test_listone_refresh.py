...
def test_refresh_orchestrates_catalog_roster_listone(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    club_counts = iter([0, 2])

    monkeypatch.setattr(
        LeagueListoneService,
        "_count_clubs_for_season",
        lambda self, year: next(club_counts),
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: calls.append("roster")
        or RosterSyncResult(
    ...
    result = service.refresh_from_provider(_access(), client=client)  # type: ignore[arg-type]
